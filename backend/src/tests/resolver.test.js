...
describe('price oracle', () => {
  ...
  test('parseQuestion extracts symbol and target price', () => {
    expect(parseQuestion('Will BTC reach $100000?')).toEqual({ symbol: 'BTC', targetPrice: 100000 });
    expect(parseQuestion('Will ETH exceed $5,000 this year?')).toEqual({ symbol: 'ETH', targetPrice: 5000 });
  ...
  test('throws for unknown coin symbol', async () => {
    ...
    await expect(
      resolve(makeMarket({ question: 'Will XRP reach $10?' }))
    ).rejects.toThrow('Unknown coin symbol');
  });
});
...
describe('sports oracle', () => {
  ...
  test('returns 0 (Yes) when team won', async () => {
    nock('https://v3.football.api-sports.io')
      .get('/teams').query(true)
      .reply(200, { response: [{ team: { id: 42, name: 'Arsenal' } }] });

    nock('https://v3.football.api-sports.io')
      .get('/fixtures').query(true)
      .reply(200, {
        response: [{
    ...
    const result = await resolve(makeMarket({ question: 'Will Arsenal win the Premier League?', category: 'sports' }));
    expect(result).toBe(0);
  });
  ...
  test('throws when team not found', async () => {
    ...
    await expect(resolve(makeMarket({ question: 'Will Arsenal win?', category: 'sports' }))).rejects.toThrow(
      'Team not found'
    );
  });
  ...
  test('throws when question cannot be parsed', async () => {
    await expect(resolve(makeMarket({ question: 'Will BTC hit $100k?', category: 'sports' }))).rejects.toThrow(
      'Cannot parse team name'
    );
  });
});
...
describe('POST /api/admin/markets/:id/resolve', () => {
  ...
  test('returns 401 without token', async () => {
    const res = await request(app).post('/api/admin/markets/1/resolve').send({ winning_outcome: 0 });
  ...
  test('returns 409 when market already resolved', async () => {
    db.query.mockResolvedValueOnce({ rows: [{ ...makeMarket(), resolved: true, outcomes: ['Yes', 'No'] }] });
    const res = await request(app)
      .post('/api/admin/markets/1/resolve')
      .set('Authorization', `Bearer ${token}`)
      .send({ winning_outcome: 0 });
